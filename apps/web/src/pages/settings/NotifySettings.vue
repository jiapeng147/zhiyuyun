...
const CHANNEL_TUTORIALS = {
  webhook: {
    ...
    checklist: [
      ...
      'Webhook 地址是可由平台服务访问的公网 HTTPS 地址，不是浏览器页面地址或内网地址。',
  ...
  feishu: {
    ...
    fields: [
      { label: 'Webhook 地址', detail: '使用飞书控制台生成的机器人地址，建议整段复制，避免丢失凭证参数。' },
    ...
    checklist: [
      ...
      '如果开启了签名，当前页面填写的 secret 与飞书控制台展示的一致。',
  ...
  dingtalk: {
    ...
    steps: [
      ...
      },
      {
        ...
        detail: '如果验证失败，优先回到钉钉控制台检查安全设置是否与当前页面完全一致。',
        tip: '大多数接入失败都不是地址错，而是安全校验没对上。'
      }
    ],
    fields: [
      { label: 'Webhook 地址', detail: '使用机器人控制台生成的完整地址，地址里通常已经包含凭证参数。' },
    ...
    tip: '最省事的定位顺序是：先看机器人控制台安全设置，再核对地址和签名密钥，最后才去改模板内容。'
  },
  wechat_work: {
    icon: 'notifyWechat',
    eyebrow: '适合稳定的企业群通知',
  ...
  pushplus: {
    ...
    fields: [
      ...
      { label: '消息模板', detail: '推荐用短句模板，方便在移动设备通知栏直接看清重点。' }
    ],
    checklist: [
      '凭证对应的 PushPlus 账号已经正常激活，并且接收端可用。',
      '验证通知已经真实出现在手机或接收设备上，而不是只有页面显示成功。',
      '模板足够简短，移动设备一屏内就能看到核心信息。'
    ],
  ...
  email: {
    ...
    steps: [
      {
        title: '先在邮箱控制台开启 SMTP 服务',
        detail: '常见邮箱都需要在网页控制台手动开启 SMTP，并生成授权码或应用专用密码。',
    ...
    checklist: [
      '邮箱控制台已经开启 SMTP，当前填写的是授权码或应用密码，不是普通登录密码。',
...
const EVENT_GROUPS = [
  ...
  },
  {
    ...
    icon: 'notifyReport',
    tone: 'blue',
    events: ['账单待支付', '账单支付确认', '账单订单关闭', '付款凭证提交', '账单退款确认', '套餐到期提醒', '套餐已到期', 'AI 额度预警', '账号配额预警', '套餐权益提醒']
  }
]
...
.notify-channel-item:hover {
  ...
  box-shadow: 0 12px 24px rgba(128, 64, 36, 0.08);
}
...
.notify-tutorial-hero.purple {
  background: linear-gradient(135deg, rgba(255, 141, 92, 0.12), rgba(255, 255, 255, 0.98));
}
...
.notify-tutorial-icon.indigo {
  background: linear-gradient(135deg, rgba(107, 107, 107, 0.14), rgba(255, 169, 132, 0.18));
  color: #e1774a;
}
...
.notify-tutorial-icon.purple {
  background: linear-gradient(135deg, rgba(255, 141, 92, 0.14), rgba(214, 164, 255, 0.2));
  color: #e27a4d;
}
